...
vorpal
  ...
  .action(function(args, callback) {
    ...
    let newProperty = propertyEval('({' + args.keyValue + '})');
...
vorpal
  ...
  .action(function(args, callback) {
    if (has(args, 'setAll')) {
      ...
      let propertiesAsObject = propertyEval('(' + newPropertiesString + ')');
...
vorpal
  ...
  .action(function(args, callback) {
    ...
    const ANNOUNCE = args.options.announce;
    let registry = registries[args.registry];
    return registry.checkFor(args.project, args.version)
      .then(
        res => {
          ...
          if (ANNOUNCE) {
            if (res){
              vorpal.log(`${chalk.green('√')} ${args.project} ${args.version} ${chalk.green("is available")} on ${args.registry}`);
            } else {
              vorpal.log(`${chalk.red('?')} ${args.project} ${args.version} ${chalk.red("not found")} on ${args.registry}`);
            }
          }
          vorpal.log(res);
        },
